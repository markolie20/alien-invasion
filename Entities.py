...
class Player:
    ...
    def boundary(self, setting):
        # left boundary
        if self.player_position_x <= 0:
            self.player_speed_left = 0
        # right boundary
        elif self.player_position_x >= setting.screen_width - \
                self.player_width:
            self.player_speed_right = 0
        # upper boundary
        if self.player_position_y <= 0:
            self.player_speed_up = 0
        # bottom boundary
        elif self.player_position_y >= setting.screen_height - \
                self.player_height:
            self.player_speed_down = 0
